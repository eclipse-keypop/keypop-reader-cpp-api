 ...
 * a CardSelectionExtension.<br> A card selection case targets a specific card.
...
public:
    ...
    /**
     * Requests the closing of the physical channel at the end of the execution
     * of the card selection request.
     *
     ...
     * string format.
     *
     * <p>This string can be imported into the same or another card selection
     ...
     *
     * <p>The string must have been exported from a card selection manager via
    ...
    /**
     * Schedules the execution of the prepared card selection scenario as soon
     * as a card is presented to the provided ObservableCardReader.
     *
     * <p>calypsonet::terminal::reader::CardReaderEvent are pushed to the
     ...
     * std::string&).
     *
     * <p>Prerequisite: the card selection scenario must first have been
     * processed via the processCardSelectionScenario(const
     * std::shared_ptr<CardReader>) or
     * parseScheduledCardSelectionsResponse(const
     * std::shared_ptr<ScheduledCardSelectionsResponse>) method.
     *
     * <p>Caution: if the local environment does not have the card extensions
     * involved in the selection scenario, then methods
     * processCardSelectionScenario(const std::shared_ptr<CardReader>) and
     * parseScheduledCardSelectionsResponse(const
     * std::shared_ptr<ScheduledCardSelectionsResponse>) will not be able to
     * interpret the content of the result, and consequently, the content of the
     * result object CardSelectionResult will not contain any active selection.
     * It will then be necessary to export the processed scenario in order to
     * import it and interpret it correctly by a card selection manager that has
     * all the card extensions involved in the selection scenario.
     *
     * @return A non-null string.
     * @throw IllegalStateException If the card selection scenario has not yet
     * been processed or has failed.
    ...
    /**
     * Imports a previously exported processed card selection scenario in string
     * format and returns the card selection result.
     *
     * <p>Prerequisites:
     *
     * <ul>
     *   <li>the string must have been exported from a card selection manager
     * via the method exportProcessedCardSelectionScenario(), <li>the local
     * environment must have the card extensions involved in the card selection
     *       scenario,
     *   <li>the current manager must first be configured with the same card
     * selection scenario as the manager that was used to export the processed
